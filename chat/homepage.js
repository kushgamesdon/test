...
document.addEventListener("DOMContentLoaded", () => {
  ...
  auth.onAuthStateChanged((user) => {
    if (user) {
      ...
      loadPosts(); // Load posts when user is logged in
...
// Load posts function for all users
function loadPosts() {
  ...
  db.collection("users").get()
    .then((usersSnapshot) => {
      if (usersSnapshot.empty) {
        postsContainer.innerHTML = "<p>No users available.</p>";
        console.log("No users found in the collection."); // Debug log
        return;
      }

      // Create an array of promises for fetching posts from each user
      const postsPromises = [];
      usersSnapshot.forEach(userDoc => {
        const userId = userDoc.id;
        // Fetch posts for each user
        const postsRef = db.collection("users").doc(userId).collection("posts");
        const postsPromise = postsRef
          .orderBy("timestamp") // Adjust as necessary
          .limit(10) // Fetch only 10 posts per user
          .get()
          .then(querySnapshot => {
            if (querySnapshot.empty) {
      ...
      // Once all posts are fetched, process and display them
      Promise.all(postsPromises)
        .then(allPostsArrays => {
          ...
          if (allPosts.length === 0) {
            ...
            console.log("No posts found in the collection."); // Debug log
            return;
          }

          allPosts.forEach(post => {
            console.log("Post data:", post); // Debug log
            const postElement = createPostElement(post, post.postId);
    ...
    .catch((error) => {
      ...
      postsContainer.innerHTML = "<p>Error fetching users. Please try again later.</p>";
    });
}
...
function createPostElement(post, postId) {
  ...
  postDiv.innerHTML = `
    ...
    <p><strong><span class="likes-count">${post.likes || 0}</span> likes</strong></p>
  ...
  likeButton.addEventListener("click", () => {
    const currentUser = auth.currentUser;
    if (currentUser) {
      const postRef = db.collection("users").doc(post.userId).collection("posts").doc(postId);
